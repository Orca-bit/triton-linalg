...
OpFoldResult mlir::triton::canonicalizeOpFoldResult(OpFoldResult in) {
  if (mlir::isa<Attribute>(in))
    return in;
  return getAsOpFoldResult(mlir::cast<Value>(in));
}
...
Value mlir::triton::materializeOpFoldResult(OpBuilder &builder, Location loc,
  ...
  if (auto value = mlir::dyn_cast<Value>(opFoldResult))
    return value;
  auto attr = mlir::cast<IntegerAttr>(mlir::cast<Attribute>(opFoldResult));
