...
由于 Triton 升级到 v3.6.0，配套的 LLVM 进行了升级，本项目进行了一系列接口适配和重构。以下是主要更改及其背后的原因。
...
## 5. 编译与测试步骤 (Compilation and Testing Steps)

为了确保升级后的代码正确性，请按照以下步骤进行编译和验证。

### 5.1 环境配置与子模块更新
确保 `triton` 子模块已更新到 `v3.6.0` 且工作目录干净：
```bash
# 更新子模块
...
### 5.2 编译项目
进入构建目录并编译核心转换工具 `triton-linalg-opt`：
```bash
# 进入构建目录 (根据实际环境调整路径)
cd triton/build/cmake.macosx-11.0-arm64-cpython-3.11

# 编译工具
ninja triton-linalg-opt
```

### 5.3 运行功能验证测试
使用提供的 `test_add_kernel.mlir` 验证 `Triton -> Linalg` 的转换逻辑：
```bash
# 在构建目录下运行
./third_party/triton_linalg/bin/triton-linalg-opt \
  --triton-to-linalg \
  ../../../test_add_kernel.mlir
```

**预期结果**：
输出的 MLIR 代码应成功将 `tt.load`、`tt.store` 等算子转换为 `linalg.generic`、`linalg_ext.pad` 以及相关的 `bufferization` 算子，且不应报错。
